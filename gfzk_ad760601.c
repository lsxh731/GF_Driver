...
#define AD760601_EIMD31_BUSY				IMX_GPIO_NR(3,31)
#define AD760601_EIMD22_RESET				IMX_GPIO_NR(3,22)
//#define AD760601_EIMDA14_BUSY				IMX_GPIO_NR(3,18)
struct mutex ad760601_lock;

unsigned int ev_canread;
DECLARE_WAIT_QUEUE_HEAD(read_waitq);//注册一个等待队列read_waitq
...
void ad760601_reset()
{
	...
	ret = -1;
	//printk("************************ %s\n", __FUNCTION__);
	if(gpio_request(AD760601_EIMD22_RESET, "ad760600_RESET"))
                pr_err("failed to request GPK1_0 for ad760602 reset control\n");
	gpio_direction_output(AD760601_EIMD22_RESET, 0);
    mdelay(3);

	gpio_set_value(AD760601_EIMD22_RESET, 0);
	ret = gpio_get_value(AD760601_EIMD22_RESET);
	printk("ad760600_RESET AD760601_EIMD22_RESET = %d !\n",ret);
	mdelay(100);
    gpio_set_value(AD760601_EIMD22_RESET, 1);
	ret = gpio_get_value(AD760601_EIMD22_RESET);
    printk("ad760600_RESET AD760601_EIMD22_RESET = %d !\n",ret);
    ...
    printk("ad760600_RESET AD760601_EIMD22_RESET = %d !\n",ret);
	mdelay(100);
    gpio_free(AD760601_EIMD22_RESET);
}
...
static irqreturn_t Ad760601Input_interrupt(int irq,void *dev_id,struct file * file)
{
	int ret;
	ssize_t			status = 0;
	//pt_count++;
	...
	mutex_lock(&ad760601_lock);
	//status = ad760602_sync_read(ad760602, count);
	// 初始化工作项
	//INIT_WORK(&mywork, mywork_handler);
	// 加入到工作队列 system_wq
	//schedule_work(&mywork);
	wake_up_interruptible(&read_waitq);   /* 唤醒休眠的进程，即调用read函数的进程 */
    ev_canread = 1; 
	mutex_unlock(&ad760601_lock);
...
ad760601_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	//-----
	//wait_event_interruptible(read_waitq, ev_canread);//将当前进程放入等待队列button_waitq中,并且释放CPU进入睡眠状态
	...
	//printk("#####ad760601_read one !#####\n");
	
	//-----
	//ev_canread = 0;//按键已经处理可以继续睡眠
...
static int __devinit ad760601_probe(struct spi_device *spi)
{
	...
	//ad760601中断引脚
	mutex_init(&ad760601_lock);
	gpio_free(AD760601_EIMD31_BUSY);
	ret = gpio_request(AD760601_EIMD31_BUSY, "ad760601_reset");
	if (ret){
		printk("\n request ad760601_reset %d failed!!! \n", AD760601_EIMD31_BUSY);
		}
		gpio_direction_input(AD760601_EIMD31_BUSY);
		gpio_free(AD760601_EIMD31_BUSY);
	ret = gpio_to_irq(AD760601_EIMD31_BUSY);
	//request_threaded_irq(ret,ErrInput_interrupt,NULL,IRQ_TYPE_EDGE_FALLING,"ErrInput_interrupt",spi);
	
	//-----
	//request_threaded_irq(ret,Ad760601Input_interrupt,NULL,IRQ_TYPE_EDGE_FALLING,"Ad760601Input_interrupt",spi);//IRQ_TYPE_EDGE_RISING
	
	printk("#####interrupt irqs = %d #####\n",ret);
